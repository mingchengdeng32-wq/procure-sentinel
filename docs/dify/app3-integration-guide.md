...
- `DIFY_BASE_URL` 实测值：`http://10.0.1.48/v1`（自托管 Dify 在服务器 10.0.1.48，注意已含 `/v1` 前缀）。
- **不要用 `http://localhost`**：localhost 仅在临时端口转发时可达，会断；真机地址是 `10.0.1.48`。
...
const BASE = process.env.DIFY_BASE_URL;        // http://10.0.1.48/v1
...
- [ ] `DIFY_BASE_URL`（=`http://10.0.1.48/v1`）/ `DIFY_APP3_KEY` 来自环境变量，全仓库 grep 不到明文 key。
...
4. **本仓库测试脚手架**：`validate_app3.py`（真机）+ `validate_app3_logic.py`（纯校验）+ `tests/test_validate_app3_logic.py`（9 个单测）+ `tests/fixtures/app3_features_sample.json`（样例输入）。复跑：`DIFY_BASE_URL=http://10.0.1.48/v1 DIFY_APP3_KEY=app-xxxx python validate_app3.py`。
